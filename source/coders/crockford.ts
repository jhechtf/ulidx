...
/**
 * @description returns a map from a character to the crockford encoding value.
 */
const CHAR_TO_QUINTET = Array.from(ALPHABET)
  .reduce((acc, chr, idx) =>  {
    acc[chr] = acc[chr.toLowerCase()] = idx; 
    return acc
  }, {
...
/**
 * @description Maps from quintets to chars. 
 */
const QUINTENT_TO_CHAR = Array.from(ALPHABET);

/**
 * 
 * @param chr the character we are converting
 * @returns a quintet value based on the crockford mapping
 */
function _charToQuintet(chr: string) {
  return CHAR_TO_QUINTET[chr];
}

function _quintetToChar(quintet: number) {
  return QUINTENT_TO_CHAR[quintet & MAX_QUINTET];
}

export class Crockford32Coder extends BaseCoder {
  constructor() {
    super({
